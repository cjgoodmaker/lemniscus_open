...
async function scanAndIndex(db) {
  if (indexing) return { indexed: 0, skipped: 0, errors: 0, message: "Indexing already in progress" };
  indexing = true;
  try {
    return await _scanAndIndex(db);
  } finally {
    indexing = false;
  }
}

async function _scanAndIndex(db) {
  ...
  // Migrate old Python manifests (used "mtime" instead of "mtimeMs")
  let migrated = false;
  for (const [, entry] of Object.entries(manifest)) {
    if (entry && entry.mtime !== undefined && entry.mtimeMs === undefined) {
      migrated = true;
      break;
    }
  }
  if (migrated) {
    console.error("Old manifest format detected — clearing for clean re-index");
    manifest = {};
...
async function main() {
  ...
  server.tool(
    ...
    async ({ source_id }) => {
      try {
        ...
        if (metrics.length === 0) {
          return toolResult({ total_readings: 0, metrics: [], hint: "No data indexed yet. Place your Apple Health export.xml in the data folder and call reindex." });
        }
        return toolResult({ total_readings: total, metrics });
      } catch (e) {
        return toolError(`Failed to list metrics: ${e.message}`);
      }
    }
  );
  ...
  server.tool(
    ...
    async ({ metric, start, end, limit, source_id }) => {
      try {
        ...
        return toolResult({ count: rows.length, readings: rows });
      } catch (e) {
        return toolError(`Failed to query readings: ${e.message}`);
      }
    }
  );
  ...
  server.tool(
    ...
    async ({ period, year, metric, source_id }) => {
      try {
        ...
        return toolError(`Failed to get summary: ${e.message}`);
      }
    }
  );
  ...
  server.tool(
    ...
    async ({ metrics, start, end, top_n, order, source_id }) => {
      try {
        ...
        return toolError(`Failed to get daily data: ${e.message}`);
      }
    }
  );
  ...
  server.tool(
    ...
    async ({ force }) => {
      try {
        if (force) {
          console.error("Force reindex: clearing all data...");
          db.clearSource(SOURCE_ID);
          saveManifest({});
        }
        ...
        return toolError(`Failed to reindex: ${e.message}`);
      }
    }
  );
